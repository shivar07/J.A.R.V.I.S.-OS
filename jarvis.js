...
function synthSound(type) {
  ...
  try {
    ...
    switch (type) {
      ...
      case "startup": {
        // Synthesise "tow tow tow beep tow" sequence
        const sequence = [
          { freq: 220, timeOffset: 0.0, duration: 0.12, type: "sine" },
          { freq: 220, timeOffset: 0.15, duration: 0.12, type: "sine" },
          { freq: 220, timeOffset: 0.30, duration: 0.12, type: "sine" },
          { freq: 880, timeOffset: 0.45, duration: 0.08, type: "sine" },
          { freq: 220, timeOffset: 0.58, duration: 0.15, type: "sine" }
        ];
        sequence.forEach((tone) => {
          ...
          gain.gain.setValueAtTime(0.02, now + tone.timeOffset);
          gain.gain.exponentialRampToValueAtTime(0.0001, now + tone.timeOffset + tone.duration);
          osc.connect(gain);
          gain.connect(ctx.destination);
...
function jarvisSpeak(text, callback) {
  if (audioMuted || !voiceSynthesisEnabled) {
...
function setupAudioToggle() {
  ...
  const tbAudioBtn = document.getElementById("taskbar-audio-btn");
  
  const updateBtnUI = () => {
    const btns = [audioBtn, tbAudioBtn];
    btns.forEach(btn => {
      if (!btn) return;
      
      const isTaskbarBtn = btn === tbAudioBtn;
      
      if (audioMuted) {
        btn.innerHTML = `
          <svg id="audio-icon" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><line x1="23" y1="9" x2="17" y2="15"></line><line x1="17" y1="9" x2="23" y2="15"></line></svg>
          ${isTaskbarBtn ? '' : 'Audio: OFF'}
        `;
        btn.style.color = "var(--accent)";
        if (!isTaskbarBtn) btn.style.borderColor = "transparent";
      } else {
        btn.innerHTML = `
          ...
          ${isTaskbarBtn ? '' : 'Audio: ON'}
        `;
        btn.style.color = "var(--primary)";
        if (!isTaskbarBtn) btn.style.borderColor = "var(--border-dim)";
      }
    });
  };
  ...
  if (tbAudioBtn) tbAudioBtn.addEventListener("click", handleToggle);
}
...
async function queryGeminiAI(userPrompt) {
  try {
    ...
    console.warn("Netlify AI function call failed:", e);
  }

  return "I am currently unable to reach the neural core servers, sir. Core protocols remain operational.";
}
...
function processTextQuery(query) {
  ...
  if (cmd.includes("introduce") || cmd.includes("who are you")) {
    reply("I am J.A.R.V.I.S., a custom robotic command assistant. I coordinate hardware diagnostics, monitor the YouTube lab feed, and organize day-to-day projects.");
  } 
    ...
    } else if (cmd.includes("portfolio") || cmd.includes("work") || cmd.includes("browser") || cmd.includes("chrome")) {
      toggleWindow("win-browser");
      reply("Launching Chrome Web Browser module, sir.");
    ...
    } else if (cmd.includes("portfolio") || cmd.includes("browser") || cmd.includes("chrome")) {
      closeWindow("win-browser");
      reply("Closing Chrome Web Browser.");
    ...
    const jokes = [
      ...
      "How many programmers does it take to change a lightbulb? None, that's a hardware problem! Ask the developer instead.",
